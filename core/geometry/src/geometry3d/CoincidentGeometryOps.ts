...
export class CoincidentGeometryQuery {
   ...
   * @param overlap unbounded segment overlap, e.g., as returned by [[coincidentSegmentRangeXY]], clamped in place
   ...
   * @return whether the segments are colinear, and if so, their overlap details or undefined
   */
  public coincidentSegmentRangeXY(pointA0: Point3d, pointA1: Point3d, pointB0: Point3d, pointB1: Point3d, restrictToBounds: boolean = true): {colinear: boolean, overlap?: CurveLocationDetailPair} {
    ...
    let overlap: CurveLocationDetailPair | undefined = CurveLocationDetailPair.createCapture(detailB0OnA, detailA0OnB);
    if (restrictToBounds)
      overlap = this.clampCoincidentOverlapToSegmentBounds(overlap, pointA0, pointA1, pointB0, pointB1);

    return {colinear, overlap};
  }
  /**
